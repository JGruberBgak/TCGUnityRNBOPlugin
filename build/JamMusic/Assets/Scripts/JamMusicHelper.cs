...
public class JamMusicHandle {
    ...
    [DllImport("JamMusic", CallingConvention = CallingConvention.StdCall)]
    ...
    internal JamMusicHandle(int key) {
        PluginKey = key;
    }
    ...
    private static JamMusicHandle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (JamMusicHandle)gch.Target;
    }
...
public class JamMusicHelper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static JamMusicHelper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("JamMusicHelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                JamMusicHelper helper = go.AddComponent<JamMusicHelper>();
                helper.Plugin = new JamMusicHandle(key);
                return helper;
            }
            return go.GetComponent<JamMusicHelper>();
        }
    }

    public JamMusicHandle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            JamMusicHandle.RegisterGlobalTransport(null);
        }
    }
}
