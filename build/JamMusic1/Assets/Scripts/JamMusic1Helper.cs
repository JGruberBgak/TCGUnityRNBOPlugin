...
public class JamMusic1Handle {
    ...
    [DllImport("JamMusic1", CallingConvention = CallingConvention.StdCall)]
    ...
    internal JamMusic1Handle(int key) {
        PluginKey = key;
    }
    ...
    private static JamMusic1Handle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (JamMusic1Handle)gch.Target;
    }
...
public class JamMusic1Helper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static JamMusic1Helper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("JamMusic1HelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                JamMusic1Helper helper = go.AddComponent<JamMusic1Helper>();
                helper.Plugin = new JamMusic1Handle(key);
                return helper;
            }
            return go.GetComponent<JamMusic1Helper>();
        }
    }

    public JamMusic1Handle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            JamMusic1Handle.RegisterGlobalTransport(null);
        }
    }
}
