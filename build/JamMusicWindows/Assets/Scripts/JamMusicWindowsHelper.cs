...
public class JamMusicWindowsHandle {
    ...
    [DllImport("JamMusicWindows", CallingConvention = CallingConvention.StdCall)]
    ...
    internal JamMusicWindowsHandle(int key) {
        PluginKey = key;
    }
    
    ~JamMusicWindowsHandle() {
    ...
    private static JamMusicWindowsHandle GetInstance(IntPtr handle) {
        GCHandle gch = GCHandle.FromIntPtr(handle);
        return (JamMusicWindowsHandle)gch.Target;
    }
...
public class JamMusicWindowsHelper : MonoBehaviour {
    private static Dictionary<int, GameObject> instances = new Dictionary<int, GameObject>();
    public static JamMusicWindowsHelper FindById(int key) {
        lock (instances) {
            GameObject go;
            if (!instances.TryGetValue(key, out go)) {
                go = new GameObject(String.Format("JamMusicWindowsHelperObject{0}", key));
                DontDestroyOnLoad(go);
                instances.Add(key, go);

                JamMusicWindowsHelper helper = go.AddComponent<JamMusicWindowsHelper>();
                helper.Plugin = new JamMusicWindowsHandle(key);
                return helper;
            }
            return go.GetComponent<JamMusicWindowsHelper>();
        }
    }

    public JamMusicWindowsHandle Plugin {
        get; internal set;
    }
    ...
    public void OnApplicationQuit() {
        ...
        if (Plugin != null) {
            ...
            JamMusicWindowsHandle.RegisterGlobalTransport(null);
        }
    }
}
